...
import { downloadClips } from '../lib/downloader';
...
export function MomentsPage({ session }: { session: Session }) {
  ...
  const searched = useMemo(
    () => filterMoments(moments, { query, tags: [] }),
    [moments, query],
  );
  ...
  const allTags = useMemo(
    () => tagCounts(moments).map((tag) => tag.name),
    [moments],
  );
  ...
  const downloadSelectedClips = useCallback(async () => {
    if (selected.length !== 1) {
      show('Select exactly one tag to download clips.');
      return;
    }

    const tag = selected[0];

    const clips = moments.filter((moment) =>
      moment.tags.some(
        (momentTag) => momentTag.toLowerCase() === tag.toLowerCase(),
      ),
    );

    if (clips.length === 0) {
      show(`No moments found for #${tag}.`);
      return;
    }
    ...
    try {
      ...
      show(
        `Downloaded ${clips.length} clip${clips.length === 1 ? '' : 's'}.`,
      );
    } catch (cause) {
  ...
  return (
    <div className="page">
        ...
        <div>
          ...
          <p className="tally">
            ...
            {moments.length >= MOMENT_LIMIT
              ? ` · showing the newest ${MOMENT_LIMIT}`
              : ''}
          </p>
        </div>
      ...
      <div className="toolbar">
        ...
        {selected.length === 1 ? (
          <button
            className="solid download-clips"
            type="button"
            disabled={downloading}
            onClick={() => void downloadSelectedClips()}
          >
            {downloading
              ? 'Downloading…'
              : `Download #${selected[0]} Clips`}
          </button>
        ) : null}
      </div>
      ...
      {status === 'error' ? (
        ...
        <section className="panel">
          ...
          <p>
            ...
            <kbd>Shift</kbd>
            <kbd>M</kbd>. The moment lands here instantly — the note and tags
            are optional.
          </p>
          <p className="muted">
            No extension installed yet? Load{' '}
            <code>packages/extension/dist</code> as an unpacked extension from{' '}
          ...
          <div className="list">
            {visible.map((moment) => (
              <MomentCard
                ...
                onNote={(note) => {
                  void setNote(moment.id, note).then(
                    (failure) => failure && show(failure),
                  );
                }}
                onTags={(tags) => {
                  void setTags(moment.id, tags).then(
                    (failure) => failure && show(failure),
                  );
                }}
                onDelete={() => {
      ...
      <div
        className={`flash${flash ? ' is-visible' : ''}`}
        role="status"
        aria-live="polite"
      >
        {flash}
